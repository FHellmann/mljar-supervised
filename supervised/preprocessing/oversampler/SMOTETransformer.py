...
class SMOTETransformer(BaseTransformer, AttributeStorage):

    def __init__(self, results_path: Optional[str] = None):
        super().__init__("smote", results_path)
    ...
    def fit(self, X: DataFrame, y: DataFrame = None, **kwargs) -> None:
        ...
        if y is None:
            raise ValueError("y must be provided to fit SMOTETransformer.")

        self.sm = SMOTE(random_state=42)

    def transform(
        self, X: DataFrame, y: [Union[DataFrame, Series]] = None, **kwargs
    ) -> Tuple[DataFrame, Union[DataFrame, Series]]:
        if y is None:
            raise ValueError("y must be provided to transform SMOTETransformer.")

        print(
            "DEBUG (SMOTETransformer.py; transform): Shapes before resampling: ",
            X.shape,
            y.shape,
        )
        X_res, y_res = self.sm.fit_resample(X, y)
        print(
            "DEBUG (SMOTETransformer.py; transform): Resampled shapes: ",
            X_res.shape,
            y_res.shape,
        )
